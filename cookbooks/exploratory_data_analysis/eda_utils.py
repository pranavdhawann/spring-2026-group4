...
from src.utils import load_stock_csv, read_yaml, working_directory_to_src
...
def get_data_paths() -> Dict[str, Path]:
    ...
    return {
        ...
        "stock_score_news_path": project_root
        / config["STOCK_SCORE_NEWS"].lstrip("../"),
    }
...
def calculate_quality_score(
    ...
    return completeness_score * completeness_weight + years_score * years_weight


def load_yoy_analysis(
    analysis_dir: Path, start_year: Optional[int] = None
) -> pd.DataFrame:
...
def aggregate_ticker_metrics(
    df: pd.DataFrame, groupby_col: str = "Ticker"
) -> pd.DataFrame:
    metrics = (
