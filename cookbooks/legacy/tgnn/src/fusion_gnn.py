...
class ConcatFusion(nn.Module):
    ...
    def __init__(self, d_ts: int = 256, d_news: int = 256, d_rep: int = 256, d_fused: int = 256):
        super().__init__()
...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(
        ...
        self.ts_attend_news = MultiHeadCrossAttention(d_model=d_ts, nhead=nhead, dropout=dropout)
        
        # Cross-attention: News attends to TS
        self.news_attend_ts = MultiHeadCrossAttention(d_model=d_news, nhead=nhead, dropout=dropout)
...
def build_fusion(config: dict) -> nn.Module:
    ...
    if mode == "concat":
        ...
        return CrossAttentionFusion(d_ts, d_news, d_rep, d_fused, nhead=4, dropout=dropout)
    else:
