...
def parse_args():
    ...
    p.add_argument(
        "--ticker", required=True, help="Ticker symbol (CSV stem, e.g. 'aal')"
    )
    p.add_argument(
        "--history-days",
        type=int,
...
def load_ticker(
    ticker: str, data_dir: Path, context_length: int, horizon: int, history_days: int
):
...
def plot(ticker, history, actual_future, pred_prices, pred_log_rets, rmse_per_step):
    ...
    if rmse_per_step:
        upper = [pivot_price] + [
            p * np.exp(r) for p, r in zip(pred_prices, rmse_per_step)
        ]
        lower = [pivot_price] + [
            p * np.exp(-r) for p, r in zip(pred_prices, rmse_per_step)
        ]
