...
def train(train_config=None):
    config = {
        ...
        "tft_config_path": "config/tft_config.yaml",
        "rand_seed": 42,
        "verbose": True,
    }
    if train_config:
    ...
    print("=" * 70)
    print("TFT BASELINE TRAINING")
    print("=" * 70)
    print("\n[1/8] Loading configurations...")
    yaml_config = read_yaml(config["yaml_config_path"])
    tft_config = read_yaml(config["tft_config_path"])

    config.update(yaml_config)
    config.update(tft_config)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        train_bar = tqdm(
            train_loader, desc=f"Epoch [{epoch:3d}/{max_epochs}] Train", leave=False
        )
    ...
    with open(model_summary_path, "w") as f:
        f.write("TFT Baseline Model Summary\n")
        f.write("Architecture: TFT (Temporal Fusion Transformer)\n")
