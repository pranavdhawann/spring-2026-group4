...
Script to train TabNet + TCN multimodal model.

This mirrors `train_baseline_multimodal.py` but uses `TabNetTCNMultiModal`
as the model, runs for 30 epochs, and uses `num_articles = 2`.
...
from src.models import TabNetTCNMultiModal
...
def train(train_config):
    config = {
        ...
        "experiment_path": "experiments/baseline/multimodal_tabnet_tcn",
    ...
    model = TabNetTCNMultiModal(model_config, len(ticker2idx), len(sector2idx))
    ...
    for epoch in range(config["epochs"]):
        ...
        for X, y in loop:
            ...
            y_true.extend(targets.detach().view(-1).cpu().numpy())
            y_pred.extend(outputs.detach().view(-1).cpu().numpy())
