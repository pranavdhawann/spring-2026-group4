...
if __package__ in {None, ""}:
    # Allow `python scripts/evaluate_chronos_2.py` from the repo root.
...
LOG_RETURN_METRICS = ["mae", "mse", "rmse", "mase", "smape", "crps"]
PRICE_METRICS = ["mae", "mse", "rmse"]
...
def _suffix_metric_results(
    metric_results: Dict[str, Any],
    suffix: str,
) -> Dict[str, Dict[str, Any]]:
    """Rename metric keys so output files carry an explicit value space."""
    return {
        "aggregated": {
            f"{metric_name}_{suffix}": value
            for metric_name, value in metric_results["aggregated"].items()
        },
        "per_series": {
            f"{metric_name}_{suffix}": value
            for metric_name, value in metric_results["per_series"].items()
        },
    }


def _merge_metric_results(*metric_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """Merge multiple metric result dicts into one aggregated/per-series view."""
    aggregated: Dict[str, float] = {}
    per_series: Dict[str, np.ndarray] = {}

    for metric_result in metric_results:
        aggregated.update(metric_result["aggregated"])
        per_series.update(metric_result["per_series"])
...
def select_plot_results(
    ...
    if requested_tickers:
        ...
        for ticker in requested_tickers:
            result = result_by_ticker.get(str(ticker).lower())
            if result is None:
        ...
        if missing_tickers:
            logger.warning(
                "Requested plot tickers not found in evaluated results: %s",
    ...
    return [all_results[idx] for idx in plot_indices]


def _reconstruct_price_space_views(
    *,
    ...
    """Reconstruct price-space views from log-return forecasts."""
    gt_prices: List[np.ndarray] = []
    median_prices: List[np.ndarray] = []
    quantile_prices: List[np.ndarray] = []
    sample_prices: List[np.ndarray] = []
    plot_contexts: List[np.ndarray] = []
    plot_context_timestamps: List[np.ndarray] = []
    plot_forecast_timestamps: List[np.ndarray] = []
    ...
    for idx, col in enumerate(target_columns):
        ...
        plot_context_timestamps.append(original_timestamps[start_idx : end_idx + 1])
        plot_forecast_timestamps.append(
            original_timestamps[end_idx + 1 : end_idx + 1 + prediction_length]
        )
        ...
        quantile_prices.append(_returns_to_price(quantile_forecast[idx], anchor_price))
    ...
    return {
        ...
        "context_values": plot_contexts,
        "context_timestamps": plot_context_timestamps,
        "forecast_timestamps": plot_forecast_timestamps,
    }
...
def evaluate_single_stock(
    ...
    metrics_to_compute = eval_cfg.get("metrics", LOG_RETURN_METRICS)
    ...
    # TODO: CRPS currently relies on quantile-to-pseudo-sample interpolation.
    # A native quantile-based CRPS would avoid this approximation error.
    ...
    plot_median_forecast = median_forecast
    plot_quantile_forecast = quantile_forecast
    plot_context_timestamps: List[np.ndarray] = []
    plot_forecast_timestamps: List[np.ndarray] = []

    # TODO: MASE scaling comes from the truncated evaluation context window.
    # Revisit if the denominator should use the full pre-holdout history.
    if target_transform == "log_return":
        ...
        plot_contexts = price_views["context_values"]
        plot_ground_truth = price_views["ground_truth"]
        plot_median_forecast = price_views["median_forecast"]
        plot_quantile_forecast = price_views["quantile_forecast"]
        plot_context_timestamps = price_views["context_timestamps"]
        plot_forecast_timestamps = price_views["forecast_timestamps"]

        log_metrics = _suffix_metric_results(
            compute_all_metrics(
                ground_truth=ground_truth,
                samples=samples,
                seasonal_errors=seasonal_errors,
                metrics_to_compute=LOG_RETURN_METRICS,
            ),
            "log_return",
        )
        price_metrics = _suffix_metric_results(
            compute_all_metrics(
                ground_truth=price_views["ground_truth"],
                samples=price_views["samples"],
                seasonal_errors=seasonal_errors,
                metrics_to_compute=PRICE_METRICS,
            ),
            "price",
        )
        metrics = _merge_metric_results(log_metrics, price_metrics)
    else:
        metrics = _suffix_metric_results(
            compute_all_metrics(
                ground_truth=ground_truth,
                samples=samples,
                seasonal_errors=seasonal_errors,
        ...
        for idx, batch in enumerate(batches):
            ...
            start_idx = total_context_len - context_len
            plot_context_timestamps.append(
                transformed_timestamps[start_idx:total_context_len]
            )
            plot_forecast_timestamps.append(transformed_timestamps[-prediction_length:])
    ...
    return {
        "ticker": ticker,
        "metrics": metrics,
        "ground_truth": plot_ground_truth[plot_idx],
        "median_forecast": plot_median_forecast[plot_idx],
        "quantile_forecast": plot_quantile_forecast[plot_idx],
        "context_values": full_context,
        "context_timestamps": plot_context_timestamps[plot_idx],
        "forecast_timestamps": plot_forecast_timestamps[plot_idx],
    }
...
def main() -> None:
    ...
    metric_names = list(next(iter(per_stock_metrics.values())).keys())
    ...
    if plot_cfg.get("enabled", False):
        ...
        plot_results = select_plot_results(all_results, plot_cfg, sample_seed)

        logger.info("Generating forecast plots for %d stocks...", len(plot_results))
        for r in plot_results:
            plot_stock_forecast(
                ...
                context_timestamps=r["context_timestamps"],
                forecast_timestamps=r["forecast_timestamps"],
            )
