...
from src.utils import load_stock_csv, read_yaml
...
def calculate_scores_from_yoy(yoy_df, start_year=2009):
    ...
    ticker_metrics["Quality_Score"] = calculate_quality_score(
        ticker_metrics["Avg_Completeness_%"], ticker_metrics["Years_of_Data"]
    )
...
def main():
    ...
    print(
        ranked_stocks[
            ["Quality_Score", "Years_of_Data", "Avg_Completeness_%", "Avg_Volatility_%"]
        ].head(20)
    )
