...
Script to predict using MultiModal model
...
def predict(predict_config):
    ...
    model = MultiModalStockPredictor(
        model_config, len(ticker2idx), len(sector2idx)
    )
    ...
    with torch.no_grad():
        ...
        for batch_idx, (X_test, y_test) in enumerate(test_loop):
            # Extract closes for plotting before prepare_batch
            closes_list = []
            for x in X_test:
                closes_list.append(np.array(x["closes_"]))
            X_closes = np.stack(closes_list)
            ...
            if batch_idx < 10:
                ...
                plot_batch_predictions(
                    ...
                    num_stocks_to_plot=min(3, X_closes.shape[0]),
                )
