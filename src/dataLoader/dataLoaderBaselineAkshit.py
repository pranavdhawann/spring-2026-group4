...
def _test_train_split(data, config):
    ...
    for ticker in ticker_data:
        # Sort by line_idx (item[1]) which is chronological in the jsonl
        ticker_data[ticker].sort(key=lambda x: x[1])

        split_idx = int(len(ticker_data[ticker]) * (1 - config["test_train_split"]))
        train_data.extend(ticker_data[ticker][:split_idx])
        test_data.extend(ticker_data[ticker][split_idx:])
