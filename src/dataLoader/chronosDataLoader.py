...
class BaselineDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ts_folder = self.config.get(
            "ts_folder", "data/multi-modal-dataset/sp500_time_series"
        )
        csv_path = os.path.join(ts_folder, f"{ticker_text.lower()}.csv")
        if os.path.exists(csv_path):
            df = pd.read_csv(csv_path)
            df["Date"] = pd.to_datetime(df["Date"], utc=True, errors="coerce")
            df["Date"] = df["Date"].dt.tz_localize(None)
            df = df.dropna(subset=["Date"]).sort_values("Date")
            window = self.config.get("HISTORY_WINDOW_SIZE", 30)
            out["ts_closes"] = df["Close"].tail(window).tolist()
        else:
            out["ts_closes"] = []
...
def _load_valid_data_points(config):
    ...
    for ticker, idx in tqdm(config["ticker2idx"].items(), desc="Loading data files"):
        ...
        try:
            ...
            print(f"     Warning: JSONDecodeError while reading {ticker}, skipping.")
            continue
