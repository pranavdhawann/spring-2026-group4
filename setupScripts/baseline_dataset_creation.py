                    ...
                    for metric_name, metric_data in metrics.items():
                        ...
                        for unit_type in ["USD", "shares", "pure", "USD/shares"]:
                            ...
                            if matched_value is None:
                                for val_entry in values:
                                    if val_entry.get("form") == form and str(
                                        val_entry.get("fy")
                                    ) == str(year):
                                        matched_value = val_entry.get("val")
                                        break
...
def merge_daily_with_financials(ts_data, date_col, financial_df, ticker):
    ...
    fin_ticker = financial_df[
        financial_df["company_name"].str.lower() == ticker.lower()
    ].copy()
    ...
    fin_ticker["filing_date"] = pd.to_datetime(
        fin_ticker["filing_date"], errors="coerce"
    )
...
def merge_with_news(merged_data, news_agg, date_col):
    ...
    final = merged_with_news.merge(
        news_agg, left_on=date_col, right_on="date", how="left"
    )
...
def create_windowed_samples(
    final_merged, ticker, sector_map, history_window=30, forecast_horizon=5, stride=1
):
    ...
    for i in range(0, len(final_merged) - total + 1, stride):
        ...
        sample = {
            ...
            "articles": [
                None if not isinstance(x, list) or len(x) == 0 else x
                for x in window["news_articles_list"]
            ],
            "time_series": [
                {
                    col.lower(): (None if pd.isna(row[col]) else float(row[col]))
                    for col in ts_cols
                }
            ...
            "table_data": [
                {
                    col: (
                        None
                        if col not in window.columns or pd.isna(row[col])
                        else str(row[col])
...
def process_ticker(ticker, config, project_root, sector_map):
    ...
    for col in ts_data.columns:
        if "date" in col.lower():
            date_col = col
            ts_data[col] = (
                pd.to_datetime(ts_data[col], utc=True)
                .dt.tz_localize(None)
                .dt.normalize()
            )
            break
    ...
    if not date_col:
        print("  No date column found in time series")
        return None
